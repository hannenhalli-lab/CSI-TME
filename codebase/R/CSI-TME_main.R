...
for (cellTypeIndex in 1:ncol(cellTypeCombinations)) {
	...
	colnames(componentMatix1) <- paste("IC.", 1:ncomp1, sep = "")
	colnames(componentMatix2) <- paste("IC.", 1:ncomp2, sep = "")
	...
	if (is.null(ncol(binMap1)) | is.null(ncol(binMap2))) {
		...
		if (all(rownames(binMap1) == rownames(binMap2))) {
			if (is_tcga) {
				...
				primary <- TCGAsampleSelect(allSampleNames, "01")
				recurrant <- TCGAsampleSelect(allSampleNames, "02")
				metastatic <- TCGAsampleSelect(allSampleNames, "06")
				
				desiredSamples <- allSampleNames[primary | recurrant | metastatic]
			} else {
		...
		componentInteractionList <- foreach(componentIndex=1:sizeCount, .packages=c('survival', 'dplyr', 'magrittr', 'TCGAutils')) %dopar% 
				{
					...
					if (is_tcga) {
						componentvalues$samples <- TCGAbarcode(componentvalues$samples, participant = TRUE)
					}
				
					clinicalData <- merge(clinical_data, componentvalues, by = 'samples')
					...
					for (binIndex in desiredBins) {
						tempMatrix = perform.cox(binaryBins = bins.binary, index = binIndex, clinicalData = clinicalData, covariates = covariates, strata = strata) #calling a function
...
write.table(significantInteractions, file = output_file, sep = "\t", quote = F, row.names = F)
